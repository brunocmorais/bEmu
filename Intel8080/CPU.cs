...
namespace bEmu.Intel8080
{
    public partial class CPU
    {
        ...
        public ushort GetNextWord()
        {
            ...
            return Util.Get16BitNumber(b1, b2);
        }
        ...
        public byte ReadByteFromMemory(ushort addr)
        {
            return state.Memory[addr];
        }

        public void WriteByteToMemory(ushort addr, byte value)
        {
            state.Memory[addr] = value;
        }

        public ushort ReadWordFromMemory(ushort addr)
        {
            byte a = state.Memory[addr];
            byte b = state.Memory[addr + 1];
            return Util.Get16BitNumber(a, b);
        }

        public void WriteWordToMemory(ushort addr, ushort word)
        {
            Util.WordTo2Bytes(word, out byte a, out byte b);
            state.Memory[addr] = b;
            state.Memory[addr + 1] = a;
        }

        public void UpdateZSP(byte value)
        {
        ...
        public byte GetByteFromRegister(Register register)
        {
            switch (register)
            {
        ...
        public ushort GetWordFromRegister(Register register)
        {
            switch (register)
            {
